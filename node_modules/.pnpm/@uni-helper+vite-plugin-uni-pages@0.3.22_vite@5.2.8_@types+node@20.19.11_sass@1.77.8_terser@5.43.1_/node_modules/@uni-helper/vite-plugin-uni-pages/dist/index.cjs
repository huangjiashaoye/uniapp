...
function checkPagesJsonFileSync(path) {
  ...
  try {
    try {
      fs__default.accessSync(path, fs__default.constants.F_OK);
    } catch {
    ...
    try {
      fs__default.accessSync(path, fs__default.constants.R_OK | fs__default.constants.W_OK);
      return true;
    } catch {
      if (!unlinkFile(path)) {
        return false;
...
async function writeFileWithLock(path, content, retry = 3) {
  ...
  try {
    try {
      release = await lockfile__default.lock(path, { realpath: false });
    } catch {
      await sleep(500);
      return writeFileWithLock(path, content, retry - 1);
    }
    await fs__default.promises.writeFile(path, content, { encoding: "utf-8" });
  } finally {
    if (release) {
      await release();
    }
...
class Page {
  constructor(ctx, path$1) {
    ...
    this.ctx = ctx;
    this.path = path$1;
    this.uri = vite.normalizePath(path$1.relativePath.replace(path.extname(path$1.relativePath), ""));
  }
  ...
  async readPageMetaFromFile() {
    try {
      const content = await fs__default.promises.readFile(this.path.absolutePath, { encoding: "utf-8" });
...
function findImports(stmts) {
  const imports = [];
  for (const stmt of stmts) {
    if (t__namespace.isImportDeclaration(stmt)) {
      imports.push(stmt);
    }
  }
  return imports;
...
class PageContext {
  ...
  async genratePagesJSON() {
    const content = await fs__default.promises.readFile(this.resolvedPagesJSONPath, { encoding: "utf-8" }).catch(() => "");
    const { pages: oldPages, subPackages: oldSubPackages, tabBar: oldTabBar } = commentJson.parse(content || "{}");
    const { pages: _, subPackages: __, tabBar: ___, ...pageJson } = this.pagesGlobConfig || {};
    const currentPlatform = uniEnv.platform.toUpperCase();
    pageJson.pages = mergePlatformItems(oldPages, currentPlatform, this.pageMetaData, "path");
    pageJson.subPackages = oldSubPackages || new commentJson.CommentArray();
  ...
  async readInfoFromPagesJSON() {
    const resolvedPagesJSONContent = await fs__default.promises.readFile(this.resolvedPagesJSONPath, { encoding: "utf-8" }).catch(() => "");
...
function mergePlatformItems(source, currentPlatform, items, uniqueKeyName) {
  const src = source || new commentJson.CommentArray();
  ...
  const result = new commentJson.CommentArray();
  const platformUsage = {};
  tmpMap.forEach((val) => {
...
exports.checkPagesJsonFileSync = checkPagesJsonFileSync;
exports.debug = debug;
