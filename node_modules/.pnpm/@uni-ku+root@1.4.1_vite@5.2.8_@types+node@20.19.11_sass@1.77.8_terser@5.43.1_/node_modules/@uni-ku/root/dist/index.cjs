...
function UniKuRoot(options) {
  ...
  const excludedPaths = toArray(options.excludePages).filter(Boolean).map((path) => node_path.resolve(rootPath, path));
  ...
  return {
    ...
    async transform(code, id) {
      ...
      const filterPage = vite.createFilter(pagesJson, excludedPaths);
