...
function stringifyComponentInfo(filepath, { from: path, as: name, name: importName }, importPathTransform) {
  if (!name)
    return void 0;
  path = utils.getTransformedPath(path, importPathTransform);
  const related = node_path.isAbsolute(path) ? `./${node_path.relative(node_path.dirname(filepath), path)}` : path;
...
async function writeDeclaration(ctx, filepath, removeUnused = false) {
  const originalContent = node_fs.existsSync(filepath) ? await promises.readFile(filepath, "utf-8") : "";
